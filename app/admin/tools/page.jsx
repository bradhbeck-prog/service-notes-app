...
const [editingGoalId, setEditingGoalId] = useState("");
  ...
  async function loadData() {
    ...
    const requestedParticipantId = new URLSearchParams(window.location.search).get("participant");
    if (requestedParticipantId && (participantRows || []).some((participant) => participant.id === requestedParticipantId)) {
      setSelectedParticipantId(requestedParticipantId);
      setGoalParticipantId(requestedParticipantId);
      setServiceParticipantId(requestedParticipantId);
      setOutcomeParticipantId(requestedParticipantId);
      setEditingParticipantId(requestedParticipantId);
    }
  }
...
async function handleUpdateGoal() {
  ...
  if (!authorized) {
    return (
      <main style={{ padding: 30, fontFamily: "Arial", maxWidth: 500, margin: "0 auto" }}>
        <h1>DreamNote Admin</h1>
        <p>Sign in with your DreamNote owner account to continue.</p>
  ...
  return (
    <main style={{ padding: 30, fontFamily: "Arial", maxWidth: 900, margin: "0 auto" }}>
      <a href="/admin" style={{ display: "inline-block", marginBottom: 12 }}>← Back to participant dashboard</a>
      <h1>DreamNote Admin — Advanced Tools</h1>
      <p style={{ color: "#444" }}>
