...
export async function GET(request, { params }) {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    return responseText("Admin PDF preview is not configured.", 500);
  }
  ...
  if (!accessToken) {
    return responseText("Sign in again before viewing a PDF.", 401);
  }

  const noteId = String(params?.noteId || "").trim();

  if (!noteId) {
    return responseText("Choose a note before viewing a PDF.", 400);
  }
  ...
  if (membershipError || !membership) {
    return responseText("You do not have permission to view admin PDFs.", 403);
  }
