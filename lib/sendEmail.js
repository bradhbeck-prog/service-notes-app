...
export async function sendPasswordHelpEmail({ to, name, actionLink }) {
  const apiKey = process.env.RESEND_API_KEY;
  ...
  const result = await resend.emails.send({
    ...
    to,
    replyTo: "bradley@supportsbroker.com",
    subject: "Choose a new DreamNote password",
    text: `Hello ${plainName},\n\nUse this secure link to choose a new DreamNote password:\n\n${actionLink}\n\nYou do not need to know your old password. If you did not request help, contact Bradley at bradley@supportsbroker.com.`,
    html: `
      <div style="font-family:Arial,sans-serif;line-height:1.55;color:#1f2937;max-width:560px;margin:0 auto">
        <h1 style="color:#149f91;font-size:28px">DreamNote password help</h1>
        <p>Hello ${safeName},</p>
        <p>Use the button below to choose a new DreamNote password. You do not need to know your old password.</p>
        <p style="margin:28px 0"><a href="${safeLink}" style="display:inline-block;background:#2e6eae;color:white;text-decoration:none;font-weight:700;padding:13px 20px;border-radius:8px">Choose a new password</a></p>
        <p style="font-size:14px;color:#596273">If the button does not work, copy and paste this address into your browser:<br><a href="${safeLink}">${safeLink}</a></p>
        <p style="font-size:14px;color:#596273">If you did not request help, contact <a href="mailto:bradley@supportsbroker.com">bradley@supportsbroker.com</a>.</p>
      </div>
    `,
  });

  if (result?.error) throw new Error(result.error.message || "The password-help email could not be sent.");
  return result;
}
